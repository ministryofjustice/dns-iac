...
def generate_metric_query(hosted_zone_id, day_offset, region):
    ...
    start_time = (datetime.now() - timedelta(days=day_offset)
                  ).strftime("%Y-%m-%d")
    ...
    print(
        f"DNS queries for the hosted zone: {hosted_zone_id} were made for {days_called} day(s) out of a possible {day_offset} day(s).\n")
...
parser = argparse.ArgumentParser(
    description='Fetch CloudWatch Metrics for Hosted Zone')
parser.add_argument(
    "-z", "--zone", help="The Hosted Zone ID (required).", required=True, default="")
parser.add_argument("-r", "--region", help="For specifying an AWS region (default = us-east-1).",
                    required=False, default="us-east-1")
parser.add_argument(
    "-d", "--days", help="For specifying the amount of days to collect data for (default = 30).", required=False, default=30)
argument = parser.parse_args()

print(
    f"Arguments: Hosted Zone ID: {argument.zone}, Days: {argument.days}, Region: {argument.region}")
